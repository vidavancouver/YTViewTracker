...
def collectTimes(interval, iterations, KEY):
    ...
    for i in range(iterations):
        print(f"[INFO] Iteration no {i+1}")
        print(f"[INFO] Waiting for {interval} seconds")
        time.sleep(interval)
        print(f"[STATUS] {datetime.now()}: collecting views")
...
def manualTimes(KEY):
    ...
    while True:
        inp = input(">>> ")
        if inp != "END":
            print(f"[STATUS] {datetime.now().time()}: collecting views")
            currentViews = getViews.main(KEY)
            dick[str(datetime.now())] = currentViews
        else:
            return(dick)